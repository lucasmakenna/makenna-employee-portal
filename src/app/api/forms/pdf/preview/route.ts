...
// POST /api/forms/pdf/preview
// Body: { docId: 'w4' | 'i9', formData: object }
// Returns the filled PDF as bytes directly — no Supabase needed.
export async function POST(request: NextRequest) {
  try {
    const { docId, formData } = await request.json();
    ...
    const filename = docId === 'w4' ? 'W-4.pdf' : 'I-9.pdf';
    ...
    console.error('[pdf-preview POST]', err);
